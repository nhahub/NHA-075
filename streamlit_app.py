...
def load_data():
    try:
        ...
        st.warning("Data file 'cleaned_data.csv' not found. Using sample data.")
        # Dummy data fallback to prevent crash
        return pd.DataFrame(
            {
                "cardio": [0, 1] * 50,
                "age_years": np.random.randint(40, 70, 100),
                "ap_hi": np.random.randint(110, 150, 100),
                "ap_lo": np.random.randint(70, 100, 100),
                "cholesterol": np.random.choice([1, 2, 3], 100),
                "glucose": np.random.choice([1, 2, 3], 100),
                "bmi": np.random.uniform(20, 35, 100),
                "pulse_pressure": np.random.randint(30, 60, 100),
                "smoke": np.random.choice([0, 1], 100),
                "physically_active": np.random.choice([0, 1], 100),
                "weight": np.random.uniform(60, 90, 100),
                "height": np.random.uniform(160, 180, 100),
            }
        )
...
if model is None or scaler is None:
    ...
    To enable live predictions, please add these files to your repository:
    - `artifacts/cvd_pipeline.joblib` (trained model)
    - `artifacts/cvd_scaler.joblib` (feature scaler)
    
    The dashboard visualizations will still work with the available data.
    """
    )
...
if model is not None and scaler is not None:
    with st.container(border=True):
        ...
        with c7:
            cholesterol = st.selectbox(
                "Cholesterol",
                options=[("Normal", 1), ("Above Normal", 2), ("High", 3)],
                format_func=lambda x: x[0],
            )
        with c8:
        ...
        if st.button("🔮 Predict Risk"):
            try:
                ...
                input_data["bmi"] = input_data["weight"] / (
                    (input_data["height"] / 100) ** 2
                )
                input_data["pulse_pressure"] = input_data["ap_hi"] - input_data["ap_lo"]
                input_data["health_index"] = (
                    input_data["physically_active"]
                    - (0.5 * input_data["smoke"])
                    - (0.5 * input_data["alcohol"])
                )
                input_data["cholesterol_gluc_interaction"] = (
                    input_data["cholesterol"] * input_data["glucose"]
                )
                input_data["hypertension"] = (
                    (input_data["ap_hi"] >= 130) | (input_data["ap_lo"] >= 90)
                ).astype(int)
                ...
                df_final[continuous_features] = scaler.transform(
                    df_final[continuous_features]
                )
    ...
    st.info(
        "Prediction feature disabled: Model files not available. Please upload model artifacts to enable predictions."
    )
...
with col_fi1:
    with st.container(border=True):
        ...
        if "cardio" in df.columns:
            ...
            colors_importance = [
                "red" if corr_with_cardio.loc[idx] > 0 else "blue"
                for idx in abs_corr.index
            ]
...
with col_fi2:
    with st.container(border=True):
        ...
        if "cardio" in df.columns:
            ...
            scatter_matrix_fig.update_traces(
                diagonal_visible=False, marker=dict(size=3, opacity=0.5)
            )
            scatter_matrix_fig.update_layout(
                height=700, legend_title_text="Outcome", dragmode="select"
            )
...
with col_rc1:
    with st.container(border=True):
        ...
        if {"ap_hi", "ap_lo", "cardio"}.issubset(df.columns):
            ...
            df_bp_cat = df.copy()
            df_bp_cat["bp_category"] = pd.Categorical(
                df_bp_cat["bp_category"], categories=cats, ordered=True
            )
            bp_counts = (
                df_bp_cat.groupby("bp_category", observed=False)["cardio"]
                .value_counts()
...
with col_rc2:
    with st.container(border=True):
        ...
        if "bmi" in df.columns:
            ...
            df_bmi_cat = df.copy()
            df_bmi_cat["bmi_category"] = pd.Categorical(
                df_bmi_cat["bmi_category"], categories=bmi_categories
            )
            bmi_grouped = (
                df_bmi_cat.groupby("bmi_category", observed=False)["cardio"]
                .value_counts()
                .unstack()
                .fillna(0)
            )
    ...
    with st.container(border=True):
        ...
        if {"cholesterol", "glucose", "cardio"}.issubset(df.columns):
            ...
            metabolic_fig.update_layout(
                title="Metabolic Risk Score Distribution", barmode="overlay", height=500
            )
...
with col_ma2:
    with st.container(border=True):
        ...
        if {"age_years", "cardio"}.issubset(df.columns):
            ...
            age_stats = (
                df.groupby("age_group", observed=False)["cardio"].mean().reset_index()
            )
            age_stats["prevalence"] = age_stats["cardio"] * 100

            age_prevalence_fig.add_trace(
                go.Scatter(
...
with st.container(border=True):
    ...
    if {"ap_hi", "ap_lo", "pulse_pressure", "cardio"}.issubset(df.columns):
        bp_box_fig = make_subplots(
            rows=1,
            cols=3,
            subplot_titles=("Systolic BP", "Diastolic BP", "Pulse Pressure"),
        )
        for status, color, label in [(0, "#27ae60", "Healthy"), (1, "#e74c3c", "CVD")]:
            subset = df[df["cardio"] == status]
            bp_box_fig.add_trace(
                go.Box(
                    y=subset["ap_hi"],
                    name=label,
                    marker_color=color,
            ...
            bp_box_fig.add_trace(
                go.Box(
                    y=subset["ap_lo"], name=label, marker_color=color, showlegend=False
                ),
                row=1,
                col=2,
            )
...
with st.container(border=True):
    ...
    if col_chosen in df.columns:
        fig_inter = px.histogram(
            ...
            labels={
                "cardio": "CVD Status",
                col_chosen: col_chosen.replace("_", " ").title(),
            },
...
# Footer
st.markdown("---")
st.markdown("**Built with Streamlit** | Data Visualization & ML Predictions")
